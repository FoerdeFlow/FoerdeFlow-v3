...
const error = computed(() => {
	if(typeof model.value !== 'number' || !Number.isInteger(model.value) || model.value < 1) {
		return t('longtermContractItem.input.ord.error.invalid')
	}
	if(props.usedOrds?.includes(model.value)) {
		return t('longtermContractItem.input.ord.error.duplicate')
	}
	return null
})
...
.kern-form-input(
	...
	label.kern-label(
		:for="id"
	) {{ $t('longtermContractItem.input.ord.label') }}
	.kern-hint(
		:id="`${id}-hint`"
	) {{ $t('longtermContractItem.input.ord.hint') }}
