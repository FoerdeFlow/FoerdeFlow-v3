...
defineExpose({
	...
	async edit(id: string) {
		const item = await $fetch(`/api/courseTypes/${id}`)
		openDialog(id, item)
	},
})
...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/courseTypes/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
			await $fetch('/api/courseTypes', {
				method: 'POST',
				body: model.value,
			})
		}
...
KernDialog(
	ref="dialog"
	:title="`Abschluss ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		CourseTypeCodeInput(v-model="model.code")
		CourseTypeNameInput(v-model="model.name")
</template>
