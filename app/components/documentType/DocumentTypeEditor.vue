...
defineExpose({
	...
	async edit(id: string) {
		const item = await $fetch(`/api/documentTypes/${id}`)
		openDialog(id, item)
	},
})
...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/documentTypes/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
...
KernDialog(
	ref="dialog"
	:title="`Vorlagenart ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		DocumentTypeCodeInput(v-model="model.code")
		DocumentTypeNameInput(v-model="model.name")
</template>
