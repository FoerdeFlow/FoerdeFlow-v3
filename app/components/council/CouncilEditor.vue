...
defineExpose({
	...
	async edit(id: string) {
		const item = await $fetch(`/api/councils/${id}`)
		openDialog(id, item)
	},
})
...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/councils/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
...
KernDialog(
	ref="dialog"
	:title="`Fachschaft ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		CouncilCodeInput(v-model="model.code")
		CouncilNameInput(v-model="model.name")
</template>
