...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/membershipTypes/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
...
KernDialog(
	ref="dialog"
	:title="`Mitgliedschaftsart ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		MembershipTypeCodeInput(v-model="model.code")
		MembershipTypeNameInput(v-model="model.name")
</template>
