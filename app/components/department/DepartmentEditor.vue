...
defineExpose({
	...
	async edit(id: string) {
		const item = await $fetch(`/api/departments/${id}`)
		openDialog(id, item)
	},
})
...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/departments/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
...
KernDialog(
	ref="dialog"
	:title="`Fachbereich ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		DepartmentCodeInput(v-model="model.code")
		DepartmentNameInput(v-model="model.name")
</template>
