...
const error = computed(() => {
	if(typeof model.value !== 'number' || !Number.isInteger(model.value) || model.value < 1) {
		return t('representationAllowanceRecipient.input.ord.error.invalid')
	}
	if(props.usedOrds?.includes(model.value)) {
		return t('representationAllowanceRecipient.input.ord.error.duplicate')
	}
	return null
})
...
.kern-form-input(
	...
	label.kern-label(
		:for="id"
	) {{ $t('representationAllowanceRecipient.input.ord.label') }}
	.kern-hint(
		:id="`${id}-hint`"
	) {{ $t('representationAllowanceRecipient.input.ord.hint') }}
