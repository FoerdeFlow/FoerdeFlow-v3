...
async function save() {
	...
	try {
		if(itemId.value) {
			await $fetch(`/api/membershipEndReasons/${itemId.value}`, {
				method: 'PUT',
				body: model.value,
			})
		} else {
...
KernDialog(
	ref="dialog"
	:title="`Grund des Mitgliedschaftsendes ${itemId ? 'bearbeiten' : 'erstellen'}`"
	:modal="modified"
	@cancel="cancel"
	@save="save"
)
	template(v-if="model")
		MembershipEndReasonCodeInput(v-model="model.code")
		MembershipEndReasonNameInput(v-model="model.name")
</template>
